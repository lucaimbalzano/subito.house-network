...
def getHouseListPageTest():
      houseListPage = []
      houseList = []
      for j in range(0,10):
            for i in range(0,30):
                  number_scraped = str(random.randrange(999, 999999))
                  houseList.append(HouseRequestDTO('name','price'+str(i),'space'+str(i),'rooms'+str(i),'floor','description','title','https://url_'+str(i),'NUMBER'+str(i)+number_scraped, False , 'RENT', 'bathrooms', 'parking', 'energyClass', 'energyHeating', 'urlUserProfile'))
